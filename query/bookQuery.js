...
function getBookByCategory(gender,major,minor,start,limit) {
    ...
    return new Promise((resolve, reject)=>{
        ...
        fetch.get(`category-info?gender=${gender}&type=hot&major=${encodeURIComponent(major)}&minor=${encodeURIComponent(minor)}&start=${start}&limit=${limit==null?pageSize:limit}`).then((res)=>{
            setTimeout(()=>{
                resolve(res.data);
            },2000)
        }).catch((err)=>{
